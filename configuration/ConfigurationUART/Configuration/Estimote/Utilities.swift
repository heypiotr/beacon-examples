...
func writeSettingsToBeacon(
    ...
    completion: @escaping (_ failedOperations: [(operationIdentifier: String, error: Error)]) -> Void) {

        var finishedOperationsCount = 0
        var failedOperations = [(operationIdentifier: String, error: Error)]()

        let completionGenerator = { (operationIdentifier: String) ->
            ((_ newValue: AnyObject?, _ error: Error?) -> Void) in return { _, maybeError in
...
func operationForSetting(
    ...
    completionGenerator: (_ operationIdentifier: String) -> ((AnyObject?, Error?) -> Void))
    -> ESTBeaconOperationProtocol! {
        ...
        case let setting as ESTSettingGPIOConfigPort0:
            return ESTBeaconOperationGPIOConfigPort0.writeOperation(withSetting: setting,
                completion: completionGenerator("GPIO port 0 mode"))

        case let setting as ESTSettingGPIOConfigPort1:
            return ESTBeaconOperationGPIOConfigPort1.writeOperation(withSetting: setting,
                completion: completionGenerator("GPIO port 1 mode"))
